...
SELECT id, charge_line_id, patient_service_product_id, responsible_party_type,
       ...
       coverage_percentage, copay_amount, non_covered_amount, status
FROM billing_charge_responsibility WHERE encounter_id = 45
...
FROM billing_payment WHERE encounter_id = 45 OR patient_id = 22
ORDER BY id;
...
FROM billing_reservation WHERE encounter_id = 45 ORDER BY id;
...
FROM billing_debit_transaction WHERE encounter_id = 45 OR patient_id = 22;
...
FROM billing_wallet WHERE patient_id = 22;
