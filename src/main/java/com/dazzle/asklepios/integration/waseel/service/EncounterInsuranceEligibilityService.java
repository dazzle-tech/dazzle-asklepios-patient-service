...
public class EncounterInsuranceEligibilityService {
    ...
    public EligibilityCheckResponse checkEncounterEligibility(Long encounterId) {
        ...
        PatientPayments payment = patientPaymentsRepository
                ...
                .orElseThrow(() -> new BadRequestAlertException(
                        "No payment found for encounterId: " + encounterId , ENTITY_NAME, "payment.notFound"
                ));

        if (!PaymentTypes.INSURANCE_PLAN.equals(payment.getPaymentTypes())) {
            throw new BadRequestAlertException(
                    "Encounter payment type is not insurance. Eligibility check is not required.", ENTITY_NAME, "paymentType.required"
            );
        }
        ...
        if (insurance == null) {
            throw new BadRequestAlertException(
                    "Insurance payment selected but no insurance plan is attached." , ENTITY_NAME, "insurance.required"
            );
        }
    ...
    private void validateInsuranceBeforeEligibility(PatientInsurance insurance) {
        ...
        if (insurance.getExpirationDate() == null ||
            ...
            throw new BadRequestAlertException("Insurance plan is expired.", ENTITY_NAME, "insurance.expired");
        }

        if (isBlank(insurance.getMemberCardId())) {
            throw new BadRequestAlertException("memberCardId is required for eligibility.", ENTITY_NAME, "memberCardId.required");
        }

        if (isBlank(insurance.getPayerNphiesId())) {
            throw new BadRequestAlertException("payerNphiesId is required for eligibility." , ENTITY_NAME, "payerNphiesId.required");
        }

        if (insurance.getPayorId() == null) {
            throw new BadRequestAlertException("payorId is required for eligibility.", ENTITY_NAME, "payorId.required");
        }

        if (insurance.getPlanId() == null) {
            throw new BadRequestAlertException("planId is required for eligibility.", ENTITY_NAME, "planId.required");
        }
    }
