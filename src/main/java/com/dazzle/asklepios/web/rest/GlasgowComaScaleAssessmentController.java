...
public class GlasgowComaScaleAssessmentController {
    ...
    @DeleteMapping("/glasgow-coma-scale-assessment/{id}")
    public ResponseEntity<Void> deleteGlasgowComaScaleAssessment(
            @PathVariable @NotNull Long id
    ) {
        LOG.debug("REST delete GlasgowComaScaleAssessment id={}", id);

        glasgowComaScaleAssessmentService.delete(id);
